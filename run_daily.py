...
REPO_ROOT = HERE.parent  # .../daily-orders/
...
def main() -> int:
    # Slack client up front so we can always post a FAILURE
    try:
    ...
    if audit["unmapped_skus"]:
        ...
        detail = (
            ...
            "Add rules to `daily-orders/references/brand_map.json`:\n\n" + "\n".join(lines)
        )
    ...
    try:
        ...
        # Fall back to a message-only post so the summary still lands in #resale
    ...
    # Side output: write the message body so it's available as a GHA artifact for debugging
    try:
